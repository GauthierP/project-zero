...
for (i = 0; i < z.length; i++) {
    ...
    var columnNumber = z[i].getElementsByClassName("column").length;
    for (j=0; j< columnNumber; ++j) {
      z[i].getElementsByClassName("column")[j].style.width = Math.round(100/columnNumber) + "%";
    }
}
...
function showDivs(n, j) {
    ...
    var dots = z.getElementsByClassName("demo");
    var captionText =  document.getElementsByClassName("caption")[j];
    if (n > x.length) {
        slideIndex = 1
    }
    if (n < 1) {
